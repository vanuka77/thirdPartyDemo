...
class PasswordInfoImpl @Inject()(userDAO: UserDAO)(implicit val classTag: ClassTag[PasswordInfo], ec: ExecutionContext)
   ...
   * @param passwordInfo user's hashed password
   */
  override def update(loginInfo: LoginInfo, passwordInfo: PasswordInfo): Future[PasswordInfo] = userDAO.find(loginInfo).flatMap { //should return new value
    case Some(user) => userDAO.update(user._id.toString, user.copy(password = Some(passwordInfo.password))).map(u=>u.get.passwordInfo)
